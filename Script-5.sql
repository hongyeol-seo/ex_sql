...
select year,category,fullname from nobel where (category = 'Chemistry' or  category = 'Peace') and fullname != "";

select year,category,fullname from nobel where category in ('Chemistry','Peace') and fullname != "";
...
select year, fullname, birth_country from nobel where category = 'Peace' and year between '1910' and '2010'and fullname != "" order by year;
...
select * from nobel where year = 1964 and not category like "Physiology%" order by fullname;
-- select * from nobel where category != 'Physiology or Medicine';
...
select distinct year from nobel where year not in(select distinct year from nobel where category = 'Physiology or Medicine');

-- 9) 노벨 의학상이 없었던 연도의 총 회수를 출력

select count(distinct year) from nobel where year not in(select distinct year from nobel where category = 'Physiology or Medicine');
...
select birth_country, count(birth_country) from nobel group by birth_country;

-- 12) 수상자의 출생 국가가 ‘Korea’인 정보 모두 출력

select * from nobel where birth_country in ('korea')

-- 13) 수장자의 출신 국가가 ‘Europe’, ‘North America’, 공백도 아닌 모든 정보 출력

select * from nobel where birth_country not in('Europe', 'North America',"");

-- 14) 수상자의 출신 국가별로 그룹화를 해서 수상 횟수가 10회 이상인 국가의 모든 정보 출력하고 국가별 수상횟수의 역순으로 출력

select birth_country,count(birth_country) from nobel group by birth_country having count(birth_country) >= 10 and birth_country != "" order by 2 desc;

-- 15) 2회 이상 수상자 중에서 fullname이 공백이 아닌 경우를 출력하는데, fullname의 오름차순으로 출력
-- - 출력 컬럼: fullname, 횟수

select fullname, count(*) from nobel group by fullname having count(*)>=2 order by 2 desc ;
